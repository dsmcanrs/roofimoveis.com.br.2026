<?php
	...
	class Template {
		...
		public function addFile($varname, $filename){
			...
			if(!$this->exists($varname)) throw new Exception("addFile: var $varname does not exist");
		...
		public function __set($varname, $value){
			...
			if(!$this->exists($varname)) throw new Exception("var $varname does not exist");
			$stringValue = $value;
			if(is_object($value)){
		...
		public function __get($varname){
			...
			throw new Exception("var $varname does not exist");
		}
		...
		public function block_exists($varname){
		...
		private function createBlocks(&$blocks) {
			...
			foreach($blocks as $parent => $block){
				foreach($block as $chield){
					...
					if(in_array($chield, $this->blocks)) throw new Exception("duplicated block: $chield");
					$this->blocks[] = $chield;
		...
		private function setBlock($parent, $block) {
			...
			else $reg = "/<!--\s*BEGIN\s+$block\s+-->\s*(\s*.*?\s*)<!--\s+END\s+$block\s*-->\s*((\s*.*?\s*))?\s*/sm";
			if(1!==preg_match($reg, $str, $m)) throw new Exception("mal-formed block $block");
			$this->setValue($name, '');
						...
						for($i = 1; $i < sizeof($property); $i++){
							if(!is_null($pointer)){
								...
								elseif(property_exists($pointer, $obj)) $pointer = $pointer->$obj;
									...
									throw new Exception("no accessor method in class ".$class." for ".$className."->".$property[$i]);
								}
							} else {
		...
		public function block($block) {
			...
			if(!in_array($block, $this->blocks)) throw new Exception("block $block does not exist");
